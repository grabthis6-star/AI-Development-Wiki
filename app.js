import { project, techGroups, techTopics, aiTopics, aiTemplates, conceptTopics, automationLevels, workflowRecipes, practicalGuides, fieldTips, toolPurposes, aiTools, toolRecipes, projectRecipes } from './knowledge-data.js'
const app=document.querySelector('#app'),toast=document.querySelector('#toast')
let customProjectPrompt=''
...
const searchItems = [
  ...
  ...projectRecipes.map((item) => ({ route: 'projects/' + item.id, icon: item.icon, title: item.title, summary: item.summary, kind: '프로젝트 예시' }))
]
...
function home(){
  app.innerHTML =
    ...
    '</section>' +
    '<section class="intro"><h2>네 곳 중 하나에서 시작하세요</h2><p>도구를 전부 공부할 필요 없이 지금 필요한 흐름 하나만 선택하면 됩니다.</p></section>' +
    '<section class="doc-grid primary-grid">' +
      ...
      '<article class="doc-card"><span>🧭</span><h2>내 프로젝트 만들기</h2><p>영어 학습·영상 제작·업무 도구 예시를 보고 내 아이디어를 첫 버전 계획으로 바꿉니다.</p><button class="btn primary" data-route="projects">프로젝트 설계하기</button></article>' +
...
function aiContent(t,index){return `<header class="detail-head"><span class="detail-icon">${t.icon}</span><span class="eyebrow">AI COLLABORATION CASE</span><h1>${t.title}</h1><p>${t.summary}</p></header><section class="block"><span class="label">실제 시행착오</span><h2>왜 문제가 됐나요?</h2><p>${t.problem}</p></section><section class="block"><span class="label">적용 예시</span><h2>요청을 이렇게 바꿔보세요</h2><div class="example-compare"><article><small>좋지 않은 요청</small><p>${t.example.bad}</p></article><article class="good"><small>개선된 요청</small><p>${t.example.good}</p></article></div></section><section class="block"><span class="label">재사용 규칙</span><h2>다음부터 어떻게 해야 하나요?</h2><p class="reuse">${t.rule}</p></section><section class="block"><span class="label">복사해서 사용</span><h2>AI에게 바로 쓰는 문장</h2><pre class="prompt">${t.prompt}</pre><button class="btn" style="margin-top:12px" data-copy-ai="${index}">문장 복사</button></section>`}
...
function listItems(items){return `<ul class="checklist">${items.map(x=>`<li>${x}</li>`).join('')}</ul>`}
function projectRecipeContent(recipe){return `<header class="detail-head"><span class="detail-icon">${recipe.icon}</span><span class="eyebrow">PROJECT RECIPE</span><h1>${recipe.title}</h1><p>${recipe.summary}</p></header><section class="block"><span class="label">사용자와 문제</span><h2>누구의 어떤 불편을 해결하나요?</h2><p><strong>사용자:</strong> ${recipe.audience}</p><p style="margin-top:8px"><strong>문제:</strong> ${recipe.problem}</p></section><section class="block"><span class="label">사용 흐름</span><h2>사용자는 어떻게 결과를 얻나요?</h2><div class="flow">${recipe.flow.map((x,i)=>`<span>${x}</span>${i<recipe.flow.length-1?'<b>→</b>':''}`).join('')}</div></section><section class="block"><span class="label">첫 버전</span><h2>지금 만들 기능</h2>${listItems(recipe.mvp)}</section><section class="block muted-block"><span class="label">범위 보호</span><h2>나중에 추가할 기능</h2>${listItems(recipe.later)}</section><section class="block"><span class="label">화면과 데이터</span><h2>무엇을 구성하나요?</h2><div class="template-grid"><article class="template"><h3>화면</h3>${listItems(recipe.screens)}</article><article class="template"><h3>저장할 정보</h3>${listItems(recipe.data)}</article></div></section><section class="block"><span class="label">완료 기준</span><h2>어디까지 되면 첫 버전이 끝인가요?</h2><p class="reuse">${recipe.completion}</p></section><section class="block"><span class="label">바로 시작</span><h2>AI에게 보내는 시작문</h2><pre class="prompt">${recipe.prompt}</pre><button class="btn" style="margin-top:12px" data-copy-project="${recipe.id}">시작문 복사</button></section>`}
function projectBuilder(){return `<section class="builder"><header><span class="eyebrow">MY PROJECT BUILDER</span><h2>내 아이디어를 첫 버전 계획으로 바꾸기</h2><p>기술을 몰라도 괜찮습니다. 다섯 가지를 적으면 AI에게 전달할 시작 계획을 만듭니다.</p></header><form id="project-builder"><div class="builder-grid"><label><span>프로젝트 이름</span><input name="name" required placeholder="예: 나만의 단어 복습 도구"></label><label><span>누가 사용하나요?</span><input name="audience" required placeholder="예: 영어 초보 학습자"></label><label class="wide"><span>어떤 불편이 있나요?</span><textarea name="problem" required placeholder="예: 모르는 표현을 검색해도 다시 복습하지 못한다"></textarea></label><label><span>사용자가 하는 행동</span><input name="action" required placeholder="예: 영어 문장을 붙여 넣는다"></label><label><span>얻어야 하는 결과</span><input name="result" required placeholder="예: 뜻과 예문이 정리된다"></label><label><span>사용할 곳</span><select name="platform"><option>웹 브라우저</option><option>PC 프로그램</option><option>모바일 중심 웹</option></select></label></div><button class="btn primary" type="submit">첫 버전 계획 만들기 →</button></form><div id="builder-result" class="builder-result hidden" aria-live="polite"></div></section>`}
function projectsPage(id=projectRecipes[0]?.id){const recipe=projectRecipes.find(x=>x.id===id)||projectRecipes[0];app.innerHTML=workspace(projectRecipes,recipe.id,'projects','PROJECT RECIPES · 아이디어에서 첫 버전까지','새 프로그램 만들기','완성된 예시를 선택해 구조를 살펴보거나, 아래 설계기에 내 아이디어를 입력해 시작 계획을 만드세요.',projectRecipeContent(recipe),projectBuilder())}
function buildCustomProject(form){
  const values=Object.fromEntries(new FormData(form).entries())
  const safe=Object.fromEntries(Object.entries(values).map(([key,value])=>[key,escapeHtml(String(value).trim())]))
  customProjectPrompt=`${values.audience}가 ${values.problem}는 문제를 해결하는 ${values.platform}용 '${values.name}'을 만들고 싶어.\n사용 흐름은 사용자가 ${values.action}하면 ${values.result}는 것이야.\n첫 버전은 입력, 결과 표시, 사용자가 결과를 확인·수정하는 기능만 구현해.\n로그인, 결제, 통계, 알림과 대규모 자동화는 추가하지 마.\n완료 기준은 실제 화면에서 이 흐름을 한 번 끝까지 실행할 수 있는 것이야.\n필요한 기술은 네가 판단하고 구현·검증·GitHub 커밋·공개 배포·브라우저 확인을 구분해서 보고해.`
  return `<span class="eyebrow">생성된 첫 버전 계획</span><h3>${safe.name}</h3><div class="project-plan"><article><small>사용자</small><p>${safe.audience}</p></article><article><small>해결할 문제</small><p>${safe.problem}</p></article><article><small>핵심 흐름</small><p>${safe.action} → ${safe.result}</p></article><article><small>사용 환경</small><p>${safe.platform}</p></article></div><h3>첫 버전 범위</h3><p>입력 → 결과 표시 → 사용자 확인·수정까지만 만듭니다. 로그인·결제·통계·알림은 다음 단계로 미룹니다.</p><h3>AI 시작문</h3><pre class="prompt">${escapeHtml(customProjectPrompt)}</pre><button class="btn" type="button" data-copy-custom>시작문 복사</button>`
}
function escapeHtml(value){return value.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;')}
function render(){const [page,arg]=location.hash.slice(1).split('/');if(page==='context')context();else if(page==='projects')projectsPage(arg);else if(page==='tech'&&arg)techDetail(arg);else if(page==='tech')techList();else if(page==='ai'&&arg)aiDetail(Number(arg));else if(page==='ai')aiList();else if(page==='concept'&&arg)conceptDetail(arg);else if(page==='concepts')conceptList();else if(page==='automation')automationPage();else if(page==='practical'&&arg)practicalDetail(arg);else if(page==='practical')practicalList();else if(page==='tool'&&arg)toolDetail(arg);else if(page==='tools')toolsList(arg||'all');else home();bind();setActiveTab(page);scrollTo(0,0)}

function setActiveTab(page){
  const active=page==='context'?'context':page==='projects'?'projects':page==='ai'?'ai':page==='tech'?'tech':['tools','tool','concepts','concept','automation','practical'].includes(page)?'reference':''
...
function bind(){
  ...
  document.querySelectorAll('[data-copy-project]').forEach(x=>x.onclick=()=>copy(projectRecipes.find(t=>t.id===x.dataset.copyProject).prompt))
  bindSearchResults()

  const builder=document.querySelector('#project-builder')
  if(builder)builder.onsubmit=(event)=>{
    event.preventDefault()
    const result=document.querySelector('#builder-result')
    result.innerHTML=buildCustomProject(builder)
    result.classList.remove('hidden')
    const copyButton=result.querySelector('[data-copy-custom]')
    if(copyButton)copyButton.onclick=()=>copy(customProjectPrompt)

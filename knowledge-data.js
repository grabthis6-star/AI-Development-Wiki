...
export const fieldTips = [
  {icon:'🔤',title:'이미지와 글자를 따로 만들기',tip:'AI 이미지 안에 정확한 한글을 억지로 생성하지 말고, 글자 없는 배경을 만든 뒤 HTML·Canvas·편집 도구로 문구를 입힙니다.',action:'배경 이미지, 표시 문구, 글꼴·위치·색상을 별도 데이터로 관리합니다.',caution:'이미지 생성 모델의 글자 품질은 도구와 시점에 따라 달라지므로 공식 기능과 짧은 샘플을 먼저 확인합니다.'},
  {icon:'🧩',title:'템플릿을 먼저 만들고 내용만 교체하기',tip:'매번 전체 화면이나 영상을 새로 만들기보다 검증된 레이아웃 하나를 만든 뒤 이미지·대사·시간만 바꿉니다.',action:'템플릿 ID와 교체 가능한 항목을 JSON에 적고, 새 결과는 데이터만 바꿔 생성합니다.',caution:'템플릿이 완성되기 전에 대량 생성을 시작하지 않습니다.'},
  {icon:'📓',title:'피드백을 프로젝트 문서에 누적하기',tip:'AI에게 같은 실수를 반복해서 설명하지 않도록 승인된 규칙과 실패 사례를 Markdown 문서에 남깁니다.',action:'피드백을 사실·선호·금지사항·검증법으로 구분해 컨텍스트 문서나 AGENTS.md에 추가합니다.',caution:'일회성 감상이나 오래된 규칙까지 모두 넣으면 오히려 충돌하므로 현재도 유효한 내용만 유지합니다.'},
  {icon:'📦',title:'대량 작업은 3개 샘플부터 시작하기',tip:'수백 개를 바로 생성하지 말고 3개 샘플로 품질·시간·비용·파일명을 먼저 검증합니다.',action:'샘플 성공 후 10개, 50개처럼 단계적으로 늘리고 동시 실행 수와 재시도를 제한합니다.',caution:'병렬 실행은 속도뿐 아니라 API 제한·메모리·비용도 함께 증가시킬 수 있습니다.'},
  {icon:'🔁',title:'실패 지점부터 다시 시작할 수 있게 만들기',tip:'한 장면의 실패 때문에 전체 작업을 처음부터 다시 돌리지 않도록 각 단계의 상태와 결과를 저장합니다.',action:'항목별 상태를 대기·진행·완료·실패로 기록하고 실패 항목만 재실행하는 버튼이나 명령을 둡니다.',caution:'성공 여부를 파일 존재만으로 판단하지 말고 크기·형식·내용도 확인합니다.'},
  {icon:'🧑‍🤝‍🧑',title:'도구마다 잘하는 일을 나누기',tip:'한 AI에게 조사·글쓰기·코딩·브라우저 검증을 전부 맡기기보다 작업에 맞는 도구를 연결합니다.',action:'조사, 구조화, 구현, 렌더링, 검수의 담당과 전달 파일을 먼저 정합니다.',caution:'도구가 많아질수록 전달 오류와 권한 위험도 늘어나므로 최소 구성으로 시작합니다.'},
  {icon:'🎯',title:'초벌 자동화와 최종 품질을 분리하기',tip:'AI는 반복되는 기본 틀과 문제 후보를 만들고, 사람은 의도·리듬·사실·저작권·최종 사용감을 결정합니다.',action:'자동 생성이 끝나는 지점과 사람이 승인해야 다음 단계로 가는 품질 관문을 명시합니다.',caution:'검수 단계를 없앤 완전 자동화를 성공 기준으로 삼지 않습니다.'}
]
